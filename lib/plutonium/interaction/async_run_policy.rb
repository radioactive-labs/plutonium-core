...
module Plutonium
  module Interaction
    ...
    #
    # Named +AsyncRunPolicy+ rather than +AsyncRuns::Policy+ so ActionPolicy's ordinary
    ...
    # +default_relation_scope+, which applies +AsyncRun.associated_with(entity_scope)+
    # — and that scope filters on the tenant the run RECORDED at dispatch (see
    # AsyncRun). Re-declaring the macro to repeat that filter by hand would add a
    ...
    class AsyncRunPolicy < Plutonium::Resource::Policy
      def read? = true
