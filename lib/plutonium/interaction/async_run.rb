...
module Plutonium
  module Interaction
    ...
    class AsyncRun < ActiveRecord::Base
      ...
      # resource_url_for(a TestPostRun) has to find the AsyncRun's route config.
      #
      # "AsyncRun" rather than the shorter "AsyncRun" on purpose: a bare "AsyncRun"
      ...
      # deep STI hierarchy (B < A < AsyncRun) inherits what A declared.
      ...
      # AsyncRuns::ReapJob. +last_activity_at+ is nil until a job actually claims
      # the run (see AsyncRuns::Executor#claim!), so a run never picked up at all
      ...
      # +completed+ (see AsyncRuns::Executor#perform_targets) — the author declared
      ...
      # "Blogging::Post" — for display (see AsyncRunDefinition). Falls back to the
      ...
      # performs anything (see AsyncRuns::Executor#record_unresolved), and that
      ...
      # do. See {#record_target_failures!} and AsyncRuns::Executor#advance!, which
      ...
      #
      # AsyncRuns::ReapJob treats a run silent past +stall_after+ as dead and resumes
      ...
      # AsyncRuns::Executor#superseded?, which turns that into "no longer mine"
      ...
      #   this and AsyncRuns::Executor#claim! both spell their SQL out.
      #
      ...
      # (AsyncRun.find(id)) goes stale at the first advance! and would raise here
      ...
      # there. AsyncRuns::Executor invokes both through send to match.
      ...
      def enqueue_job
        Plutonium::Interaction::AsyncRuns::Job.perform_later(id)
      rescue
