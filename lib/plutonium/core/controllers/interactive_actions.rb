  ...
  module Core
    module Controllers
      module InteractiveActions
        ...
        included do
          before_action :authorize_interactive_resource_record_action, only: %i[begin_interactive_resource_record_action commit_interactive_resource_record_action]
          before_action :authorize_interactive_resource_collection_action, only: %i[begin_interactive_resource_collection_action commit_interactive_resource_collection_action]
        end

        # GET /resources/1/actions/:interactive_action
        def begin_interactive_resource_record_action
          @action = interactive_resource_record_actions[params[:interactive_action].to_sym]
        ...
        def interactive_resource_record_actions
          @interactive_resource_record_actions ||= current_presenter.actions.except :new, :show, :edit, :destroy
        end

        def authorize_interactive_resource_record_action
          interactive_resource_record_action = params[:interactive_action]&.to_sym

          unless interactive_resource_record_actions.key?(interactive_resource_record_action)
            raise ::AbstractController::ActionNotFound, "Unknown action #{interactive_resource_record_action}'"
          end

          authorize resource_record, :"#{interactive_resource_record_action}?"
        end

        def authorize_interactive_resource_collection_action
          interactive_resource_record_action = params[:interactive_action]&.to_sym
          ...
          authorize resource_class, :"#{interactive_resource_record_action}?"
        end
      end
    end
  end
