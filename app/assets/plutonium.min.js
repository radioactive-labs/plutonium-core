...
`,e=e.substring(w.length+1),h=!0),!h){let A=this.rules.other.nextBulletRegex(L),R=this.rules.other.hrRegex(L),T=this.rules.other.fencesBeginRegex(L),x=this.rules.other.headingBeginRegex(L),P=this.rules.other.htmlBeginRegex(L);for(;e;){let D=e.split(`
`,1)[0],B;if(w=D,this.options.pedantic?(w=w.replace(this.rules.other.listReplaceNesting,"  "),B=w):B=w.replace(this.rules.other.tabCharGlobal,"    "),T.test(w)||x.test(w)||P.test(w)||A.test(w)||R.test(w))break;if(B.search(this.rules.other.nonSpaceChar)>=L||!w.trim())g+=`
