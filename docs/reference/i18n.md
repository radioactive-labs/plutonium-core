...
Actions, scopes, filters, kanban columns and wizard steps default to a humanized version of their key. Each has a convention key that takes precedence over that fallback, and a `label:` option that takes precedence over both:
...
```html
<script type="application/json" id="pu-i18n">{"clipboard":{"copied":"Copied!"},...}</script>
```

The blob is the `plutonium.js` subtree of the locale, so overriding a key in YAML changes the bundled JavaScript's text without rebuilding it. Host code can read the same blob:

```js
window.Plutonium.t("plutonium.js.turbo_confirm.confirm")
window.Plutonium.t("plutonium.js.attachment.delete", { name: "photo.png" })
```
