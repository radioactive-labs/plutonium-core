...
class RunsInstallGeneratorTest < Rails::Generators::TestCase
  ...
  tests Pu::Runs::InstallGenerator
  ...
  test "registers Plutonium::Interaction::Run and generates a controller with controller_for" do
  ...
  test "is idempotent on the route registration" do
    ...
    assert_file "packages/test_portal/config/routes.rb" do |content|
      assert_equal 1, content.scan("register_resource ::Plutonium::Interaction::Run").size
    end
  end
  ...
  test "schedules ReapJob in config/recurring.yml when solid_queue is present" do
    ...
    assert_file "config/recurring.yml" do |content|
      assert_match(/reap_stalled_interaction_runs:/, content)
      assert_match(/class: Plutonium::Interaction::Runs::ReapJob/, content)
  ...
  test "is idempotent on the recurring task" do
    ...
    assert_file "config/recurring.yml" do |content|
      assert_equal 1, content.scan("reap_stalled_interaction_runs:").size
    end
  end
