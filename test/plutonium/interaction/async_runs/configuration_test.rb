...
class Plutonium::Interaction::AsyncRuns::ConfigurationTest < ActiveSupport::TestCase
  test "default values" do
    config = Plutonium::Interaction::AsyncRuns::Configuration.new
  ...
  test "migration path is surfaced only when enabled" do
    ...
    # first.
    was_enabled = Plutonium.configuration.async_runs.enabled
    ...
    path = Plutonium.root.join("db/migrate/async_runs").to_s
    Plutonium::Migrations.register(:async_runs, path)
    ...
    Plutonium.configuration.async_runs.enabled = was_enabled
  end
end
