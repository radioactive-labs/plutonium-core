...
class Plutonium::Definition::DisplayLayoutResolutionTest < Minitest::Test
  def definition(&block)
    Class.new(Plutonium::Definition::Base, &block).new
  end

  def test_returns_nil_without_layout
    assert_nil definition {}.resolve_display_sections(%i[a b])
  end
  ...
  def test_assigns_fields_and_collects_leftovers_last_by_default
    ...
    resolved = d.resolve_display_sections(%i[name email notes secret])
  ...
  def test_ungrouped_macro_controls_position_and_options
    ...
    resolved = d.resolve_display_sections(%i[name notes])
    assert_equal %i[identity ungrouped], resolved.map { |r| r.section.key }
  ...
  def test_preserves_section_field_order
    d = definition { display_layout { section :a, :two, :one } }
    resolved = d.resolve_display_sections(%i[one two])
  ...
  def test_empty_section_kept_when_field_filtered
    ...
    resolved = d.resolve_display_sections(%i[name])
  ...
  def test_field_not_in_permitted_set_is_skipped_not_raised
    d = definition { display_layout { section :a, :nope, :name } }
    resolved = d.resolve_display_sections(%i[name])
