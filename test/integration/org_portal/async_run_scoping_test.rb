...
class OrgPortal::AsyncRunScopingTest < ActionDispatch::IntegrationTest
  ...
  test "the run's URL is entity-prefixed, and that is where dispatch redirects" do
    ...
    assert_equal "#{prefix}/async_runs/#{@mine.to_param}", resolved
    get resolved
    assert_response :success
  end

  test "a run dispatched in another tenant is not readable" do
    get "#{prefix}/async_runs/#{@theirs.to_param}"
  ...
  test "the index lists only this tenant's runs" do
    ...
    assert_match(/#{prefix}\/async_runs\/#{@mine.to_param}/, response.body)
    refute_match(/#{prefix}\/async_runs\/#{@theirs.to_param}/, response.body,
