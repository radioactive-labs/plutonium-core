...
export default class extends Controller {
  ...
  select(event) {
    ...
    // 1 year, site-wide. SameSite=Lax keeps it on top-level navigations
    // but blocks cross-site requests from carrying it along.
    const maxAge = 60 * 60 * 24 * 365
    document.cookie = `${this.cookieNameValue}=${encodeURIComponent(view)}; Path=/; Max-Age=${maxAge}; SameSite=Lax`
