...
| **[[plutonium-runs]]** | Async interactions — `dispatches_to`, the Run STI model, failure policies (`halt`/`continue`/`transactional`), authorization re-derivation at perform time, registering Run as a resource (progress page + running banner), scheduling `ReapJob` |
...
| Make a bulk/long-running interaction async (`dispatches_to`), or schedule the stalled-run reaper | **[[plutonium-runs]]** |
